...
namespace tf::constants
{
  ...
  template<typename T> constexpr T q_e = static_cast<T>(1.602176634e-19); // electron charge, coulombs
  template<typename T> constexpr T m_e = static_cast<T>(9.109383714e-31);  // electron mass, kg
  template<typename T> constexpr T m_p = static_cast<T>(1.672621925e-27);  // proton mass, kg
}
