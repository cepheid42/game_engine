...
namespace tf::particles {
  ...
  std::array<double, 6> FieldAtParticle(Particle& p, const auto& emdata)
  {
  ...
  void BorisPush::update_velocity(Particle& p, const emdata_t& emdata, const auto qdt)
  {
  ...
  void BorisPush::update_position(Particle& p) {
  ...
  void BorisPush::advance_velocity(group_t& g, const emdata_t& emdata) {
#pragma omp parallel for num_threads(nThreads) schedule(dynamic, chunkSize)
  ...
  void BorisPush::advance_position(group_t& g) {
#pragma omp parallel for num_threads(nThreads) schedule(dynamic, chunkSize)
    for (std::size_t pid = 0; pid < g.num_particles(); pid++) {
  ...
  void BorisPush::update_particles(group_t& g) {
    for (std::size_t pid = 0; pid < g.num_particles(); pid++) {
      ...
      if (inew < BC_DEPTH or inew > Nx - BC_DEPTH or knew < BC_DEPTH or knew > Nz - BC_DEPTH) {
        ...
        p.code = morton_encode(inew, j, knew);
      }
    } // end for(pid)
