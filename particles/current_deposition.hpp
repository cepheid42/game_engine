...
namespace detail
{
   ...
   struct TrajectoryAssignment
   {
      ...
      : startShape(std::move(start_func)),
        endShape(std::move(end_func)) {}
   ...
   void depositJx(auto& J, const auto& p0, const auto& p1, const auto& cids, const auto qA)
   {
      ...
      const JitTrajectory     shapeI(JitShape{x0 - ci}, JitShape{x1 - ci});
      const ReducedTrajectory shapeJ(ReducedShape{y0 - cj}, ReducedShape{y1 - cj});
      const CachedTrajectory  shapeK(CachedShape{z0 - ck}, CachedShape{z1 - ck});
      ...
      for (int j = b0[1]; j <= b1[1]; ++j)
      {
         ...
         for (int k = b0[2]; k <= b1[2]; ++k)
         {
            ...
            for (int i = b0[0]; i <= b1[0]; ++i)
            {
               ...
               // dbg(shapeI.S0(i), shapeI.S1(i), acc);
   ...
   void depositJz(auto& J, const auto& p0, const auto& p1, const auto& cids, const auto qA)
   {
      ...
      if (p0[2] == p1[2]) { return; } // no deposition in this direction
      ...
      const CachedTrajectory  shapeI(CachedShape{x0 - ci}, CachedShape{x1 - ci});
      const ReducedTrajectory shapeJ(ReducedShape{y0 - cj}, ReducedShape{y1 - cj});
      const JitTrajectory     shapeK(JitShape{z0 - ck}, JitShape{z1 - ck});
      ...
      for (int i = b0[0]; i <= b1[0]; ++i)
      {
         ...
         for (int j = b0[1]; j <= b1[1]; ++j)
         {
            ...
            for (int k = b0[2]; k <= b1[2]; ++k)
            {
               ...
               const auto val = qA * dsk * (s0i * s0j + 0.5_fp * (dsi * s0j + s0i * dsj) + third * dsj * dsi);
               // dbg(ci + i, cj + j, ck + k, val);
               // dbg(shapeI.S0(i), shapeI.S1(i), shapeJ.S0(j), shapeJ.S1(j), shapeK.S0(k), shapeK.S1(k));
   ...
   void updateJ(const auto& p, auto& emdata, const auto charge)
   {
      ...
      {
         dbg("Second Trajectory.");
         depositJx(emdata.Jx, p1, pnew, cids, x_coeff);
         // depositJz(emdata.Jz, p1, pnew, cids, z_coeff);
      }
