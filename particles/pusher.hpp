                           ...
                           const auto& ci, const auto& cj, const auto& ck)
{
   ...
   auto result = 0.0;
   for (int i = OShape::Begin; i <= OShape::End; ++i) {
      const auto& s0i = shapeI[i - OShape::Begin];
      for (int j = MShape::Begin; j <= MShape::End; ++j) {
         const auto& s0j = shapeJ[j - MShape::Begin];
         for (int k = IShape::Begin; k <= IShape::End; ++k) {
            const auto& s0k = shapeK[k - IShape::Begin];
