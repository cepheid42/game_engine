...
namespace tf::particles {
  ...
  static std::array<double, 6> FieldAtParticle(Particle& p, const auto& emdata) {
    ...
    const auto ci = x_range[0] + static_cast<double>(i) * dx;
    const auto cj = y_range[0] + static_cast<double>(j) * dy;
    const auto ck = z_range[0] + static_cast<double>(k) * dz;

    const auto xr_shapes = shapesCIC(ci - p.location[0]);
    const auto yr_shapes = shapesCIC(cj - p.location[1]);
    const auto zr_shapes = shapesCIC(ck - p.location[2]);
    const auto x_shapes = shapesQuad(ci - p.location[0]);
    const auto y_shapes = shapesQuad(cj - p.location[1]);
    const auto z_shapes = shapesQuad(ck - p.location[2]);
    ...
    const auto Ex_c = getField<0, 2, 1, 3>(emdata, idxs, {0, 1, 0, 1, -1, 1});
    const auto Ey_c = getField<1, 3, 1, 3>(emdata, idxs, {-1, 1, 0, 1, -1, 1});
    const auto Ez_c = getField<2, 3, 1, 2>(emdata, idxs, {-1, 1, 0, 1, 0, 1});

    const auto Bx_c = getField<3, 3, 1, 2>(emdata, idxs, {-1, 1, 0, 1, 0, 1});
    const auto By_c = getField<4, 2, 1, 2>(emdata, idxs, {0, 1, 0, 1, 0, 1});
    const auto Bz_c = getField<5, 2, 1, 3>(emdata, idxs, {0, 1, 0, 1, -1, 1});

    return {
      calculateShape<6>(Ex_c, ex_shapes),
  ...
  struct BorisPush {
    ...
    static constexpr std::size_t BC_DEPTH = 5; //PMLDepth - 10;
    ...
    static void update_particles(group_t& g) {
      ...
      for (std::size_t pid = 0; pid < g.num_particles(); pid++) {
        ...
        const auto inew = static_cast<std::size_t>(std::abs((p.location[0] - x_range[0]) / dx));
        const auto jnew = static_cast<std::size_t>(std::abs((p.location[1] - y_range[0]) / dy));
        const auto knew = static_cast<std::size_t>(std::abs((p.location[2] - z_range[0]) / dz));

        // std::println("{}, {}, {}", p.location[0], p.location[1], p.location[2]);
