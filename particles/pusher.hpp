...
static std::array<vec3<double>, 2> fieldAtParticle(const Particle& p, const auto& emdata, const auto qdt) {
   ...
   // using ZerothOrder = interp::InterpolationShape<interpolation_order - 1>;
   // using FirstOrder = interp::InterpolationShape<interpolation_order>;
   //
   // using ExStrategy = interp::InterpolationStrategy<FirstOrder, AssignmentShape, ReducedShape>;
   // using EyStrategy = interp::InterpolationStrategy<AssignmentShape, AssignmentShape, ZerothOrder>;
   // using EzStrategy = interp::InterpolationStrategy<AssignmentShape, FirstOrder, ReducedShape>;
   // using BxStrategy = interp::InterpolationStrategy<ZerothOrder, ReducedShape, AssignmentShape>;
   // using ByStrategy = interp::InterpolationStrategy<ReducedShape, ReducedShape, FirstOrder>;
   // using BzStrategy = interp::InterpolationStrategy<ReducedShape, ZerothOrder, AssignmentShape>;
...
struct BorisPush {
   ...
   static constexpr std::size_t BC_DEPTH = std::max(PMLDepth / 3, 1zu);
   ...
   static void update_position(Particle& p) {
      ...
      p.disabled = inew < BC_DEPTH or inew > Nx - 1 - BC_DEPTH or knew < BC_DEPTH or knew > Nz - 1 - BC_DEPTH;
      // p.disabled = new_loc[0] <= 3.0 or new_loc[0] >= 97.0 or new_loc[2] <= 3.0 or new_loc[2] >= 97.0;
