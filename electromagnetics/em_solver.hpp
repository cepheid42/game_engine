...
struct EMSolver {
   ...
   static void updateH(auto& emdata) {
      FieldUpdate<Dz, Dy, false, empty>::apply(
         mdspan_t{&emdata.Hxf[0, 0, 0], {std::extents{Nx, Ny - 1, Nz - 1}, hx_stride}},
         mdspan_t{&emdata.Eyf[0, 0, 0], {std::extents{Nx, Ny - 1, Nz - 1}, ey_stride}},
         mdspan_t{&emdata.Ezf[0, 0, 0], {std::extents{Nx, Ny - 1, Nz - 1}, ez_stride}},
         empty{}, dtdz_mu, dtdy_mu, 0.0
      );

      FieldUpdate<Dx, Dz, false, empty>::apply(
         mdspan_t{&emdata.Hyf[0, 0, 0], {std::extents{Nx - 1, Ny, Nz - 1}, hy_stride}},
         mdspan_t{&emdata.Ezf[0, 0, 0], {std::extents{Nx - 1, Ny, Nz - 1}, ez_stride}},
         mdspan_t{&emdata.Exf[0, 0, 0], {std::extents{Nx - 1, Ny, Nz - 1}, ex_stride}},
         empty{}, dtdx_mu, dtdz_mu, 0.0
      );

      FieldUpdate<Dy, Dx, false, empty>::apply(
         mdspan_t{&emdata.Hzf[0, 0, 0], {std::extents{Nx - 1, Ny - 1, Nz}, hz_stride}},
         mdspan_t{&emdata.Exf[0, 0, 0], {std::extents{Nx - 1, Ny - 1, Nz}, ex_stride}},
         mdspan_t{&emdata.Eyf[0, 0, 0], {std::extents{Nx - 1, Ny - 1, Nz}, ey_stride}},
         empty{}, dtdy_mu, dtdx_mu, 0.0
      );
   }
