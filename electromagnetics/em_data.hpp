...
namespace tf::electromagnetics {
  struct EMData {
    ...
    : Ex(nx - 1, ny, nz), Jx(nx - 1, ny, nz), Cexe(nx - 1, ny, nz), Cexhy(nx - 1, ny, nz), Cexhz(nx - 1, ny, nz), Cjx(nx - 1, ny, nz),
      ...
      Bx(nx, ny - 1, nz - 1), Chxey2(nx, ny - 1, nz - 1), Chxez2(nx, ny - 1, nz - 1),
      By(nx - 1, ny, nz - 1), Chyex2(nx - 1, ny, nz - 1), Chyez2(nx - 1, ny, nz - 1),
      Bz(nx - 1, ny - 1, nz), Chzex2(nx - 1, ny - 1, nz), Chzey2(nx - 1, ny - 1, nz),
    ...
    auto get(const std::size_t i, const std::size_t j, const std::size_t k) const {
      if constexpr (F == 0) {
        ...
        return Bx(i, j, k);// + Bx_app(i, j, k);
      }
      else if constexpr (F == 4) {
        return By(i, j, k);// + By_app(i, j, k);
      }
      else if constexpr (F == 5) {
        return Bz(i, j, k);// + Bz_app(i, j, k);
      } else {
        assert(false);
      }
    }
