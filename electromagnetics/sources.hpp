...
struct BlackmanHarris final : TemporalSource {
   ...
   : omega((2.0 * constants::pi * constants::c) / (Nl * dx_)),
...
struct GaussianBeam : CurrentSource {
   ...
   void apply(const double t) const {
      ...
      //    for (size_t j = y0; j < y1; ++j) {
      //       for (size_t k = z0; k < z1; ++k) {
      //          (*Hx)(i, j, k) -= coeffs[k - z0] * H_src / constants::eta0;
      //          (*Hz)(i, j, k) += coeffs[k - z0] * H_src / constants::eta0;
      //       }
      //    }
      // }
   }
...
struct RamanSource : CurrentSource {
   using array_t = Array3D<double>;
   using offset_t = std::array<std::size_t, 6>;

   RamanSource(array_t* const f_,
               const double freq_,
               const double E0_,
               const double t0_,
               const double sigma_,
   ...
   void apply(const double t) const {
      const auto& [x0, x1, y0, y1, z0, z1] = src.offsets;
      const auto val = E0
               * std::sin(2.0 * constants::pi * freq * t)
               * std::exp(-0.5 * std::pow((t - t0) / sigma, order));

      if (std::abs(val) <= 1e-6) return;
...
void add_rmf_antennas(auto& emsolver, const auto rmf_params) {
   ...
   const auto omega = 2.0 * constants::pi * freq;
   const auto delay_lr = period / 4.0;

   const auto amp_m = -amp;
   const auto amp2 = 2.0 * amp;
   ...
   constexpr auto phase_tb = -constants::pi_over_2;
   constexpr auto start_tb = 0.0;
   constexpr auto phase_lr = 0.0;
...
void add_raman_source(auto& em) {
   using temporal_vec = std::vector<std::unique_ptr<TemporalSource>>;

   // Add seed pulse
   constexpr auto x0_seed = 0;
   constexpr auto x1_seed = (Nx - 1) - x0_seed - 1;
   constexpr auto y0_seed = 0;
   constexpr auto y1_seed = Ny - y0_seed - 1;
   constexpr auto z0_seed = PMLDepth + 10lu;
   constexpr auto z1_seed = z0_seed + 1;

   constexpr auto freq_seed = constants::c / 1.17e-6;
   constexpr auto amp_seed = 1.0e5 * 4.0e5; // kV/cm -> V/m
   constexpr auto peak_seed = 1.0e-9 * 75.0e-6; // ns -> s
   constexpr auto sigma_seed = 1.0e-9 * 23.0e-6; // ns -> s
   constexpr auto order_seed = 2.0;
   em.emdata.raman.emplace_back(
      &em.emdata.Ex,
      freq_seed,
      amp_seed,
      peak_seed,
   ...
   // Add pump pulse
   constexpr auto x0_pump = 0;
   constexpr auto x1_pump = (Nx - 1) - x0_pump - 1;
   constexpr auto y0_pump = 0;
   constexpr auto y1_pump = Ny - y0_pump - 1;
   constexpr auto z0_pump = Nz - PMLDepth - 10lu;
   constexpr auto z1_pump = z0_pump + 1;

   constexpr auto freq_pump = constants::c / 1.053e-6;
   constexpr auto amp_pump = 1.0e5 * 6.93e5; // kV/cm -> V/m
   constexpr auto peak_pump = 1.0e-9 * 2700.0e-6; // ns -> s
   constexpr auto sigma_pump = 1.0e-9 * 2000.0e-6; // ns -> s
   constexpr auto order_pump = 10.0;
   em.emdata.raman.emplace_back(
      &em.emdata.Ex,
      freq_pump,
      amp_pump,
